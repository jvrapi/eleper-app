...
const HospitalizationList: React.FC = () => {
	...
	const { hospitalizationIcon } = pageIcons;
	...
	async function onDeleteItems() {
		...
		try {
			...
			getData();
			showMessage({
				message: 'Internações excluídas com sucesso!',
				type: 'success',
				icon: 'success',
			});
		} catch {
			showMessage({
				message: 'Não consegui excluir as internações, pode tentar de novo?',
				type: 'danger',
				icon: 'danger',
			});
		} finally {
							...
							<ScrollView style={styles.scroll} refreshControl={<RefreshControl refreshing={loading} onRefresh={onRefresh} />}>
								{items.map((item, i) => (
									<Card key={i} style={[styles.card, styles.shadow]} onLongPress={() => onLongPressCard(i)} onPress={() => onPressCard(i)}>
										...
										{!multiSelect && hospitalizationIcon}
										{multiSelect && (
